...
for i in range(len(w)):
    ...
    c=a/(math.sqrt(b*f))
    w[i].append(c)
    # sym is a graph represented by adjacency list for a given vertex with the c value 
...
# initalzing community with -1 
for i in range(len(l)):
    comm.append(-1)
g=0

# list of edge with c value --> w 
for i in range(len(w)):
    a=w[i][0] # vertex one 
    b=w[i][1] # vertex two 
       # v1 ka com     # v2 ka com  agar initialized hai to 
    if(comm[a]> -1 and comm[b]> -1):
        sym[a]=[subl for subl in sym[a] if subl[0] != b]
        sym[b]=[subl for subl in sym[b] if subl[0] != a]
        continue
    # find the neighbour with the maximum c value for vertex one
    c=max(sym[a], key=lambda x: x[1])
    # fine the neighbout with the maximum c value for vertex two 
...
# g is the count of commmunities in the grpah 
for i in range(g): 
    # initializing g number of communities in the community_list_initial 
    community_list_intial.append([])

for i in range(len(l)):
    # setting in the vertices belonging to the speicfic community as per their g label. 
    # this means that all the list in the community_list_initial consists of vertices belonging to the community 
...
for i in range(len(community_list_intial)):
    # for all the vertices in the current commuinty 
    for j in range(len(community_list_intial[i])):
        # for all the vertices after the current vertex. 
...
# ,node_color= color_map_com, node_size=1000,orientation = 'landscape',dpi = 200,with_labels=False, font_color='white'
    nx.draw(graph, node_color= color_map, node_size=1000,with_labels=False, font_color='white')
...
#             print (result)
            ...
            if activate_node_cal(result) == N:
                break
#             print(color_map)
            # node_color= color_map, node_size=1000,orientation = 'landscape',dpi = 200,with_labels=False, font_color='white')
            nx.draw(graph, node_color= color_map, node_size=1000,with_labels=False, font_color='white')
            plt.show()
